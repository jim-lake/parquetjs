  ...
  before(async function () {
    ...
    for (let i = 0; i < reader.metadata.row_groups.length; i++) {
      ...
      for (let j = 0; j < rowGroup.columns.length; j++) {
        ...
        try {
          await reader.envelopeReader.readOffsetIndex(column.meta_data.path_in_schema.join(','), rowGroup, {
            cache: true,
          });
          await reader.envelopeReader.readColumnIndex(column.meta_data.path_in_schema.join(','), rowGroup, {
            cache: true,
          });
