...
describe('ParquetShredder', function () {
  it('should shred a single simple record', function () {
    ...
    assert.deepEqual(
      colData.name.values.map((x) => x.toString()),
      ['apple']
    );
  ...
  it('should shred a list of simple records with repeated scalar fields', function () {
    ...
    assert.deepEqual(
      colData.colours.values.map((x) => x.toString()),
  ...
  it('should shred a nested record without repetition modifiers', function () {
    ...
    assert.deepEqual(
      colData[['stock', 'warehouse']].values.map((x) => x.toString()),
      ['A', 'B']
    );
  ...
  it('should materialize a nested record with nested repeated fields', function () {
    ...
    buffer.columnData[['stock', 'warehouse']] = {
      dlevels: [1, 1, 1, 0, 1],
      rlevels: [0, 1, 0, 0, 0],
      values: [Buffer.from([65]), Buffer.from([66]), Buffer.from([88]), Buffer.from([67])],
      count: 5,
    };
    ...
    assert.deepEqual(records[2], { name: 'kiwi', price: 99.0, stock: null });

    assert.deepEqual(records[3], { name: 'banana', stock: [{ quantity: null, warehouse: 'C' }], price: 42.0 });
  });
  ...
  it('should materialize a static nested record with blank optional value', function () {
    ...
    assert.deepEqual(records[0], { fruit: { name: 'apple', colour: null } });
  });
});
