...
class SplitBlockBloomFilter {
  ...
  private static readonly BITS_PER_BLOCK: number =
    SplitBlockBloomFilter.WORDS_PER_BLOCK * SplitBlockBloomFilter.WORD_SIZE;
  ...
  private static readonly LOWER_BOUND_BYTES =
    (SplitBlockBloomFilter.NUMBER_OF_BLOCKS * SplitBlockBloomFilter.BITS_PER_BLOCK) / 8;
  ...
  setOptionNumFilterBytes(numBytes: number): SplitBlockBloomFilter {
    ...
    if (numBytes < SplitBlockBloomFilter.LOWER_BOUND_BYTES || numBytes > SplitBlockBloomFilter.UPPER_BOUND_BYTES) {
      console.error(
        `invalid numBits. Must be > ${SplitBlockBloomFilter.LOWER_BOUND_BYTES} and < ${SplitBlockBloomFilter.UPPER_BOUND_BYTES}`
      );
      return this;
    }
